/*
  ...
  @ marks a module which mostly exists as a wrapper
    ...
    + sp_format        "a type-safe {:fg cyan} replacement",
  SP_FMT_CSTR("printf") sp_fmon          os native filesystem watching
    + sp_fs            path manipulation, filesystem, common system paths (e.g.
  appdata)
    ...
    + sp_str           ptr + len strings, no null termination, fundamental c
  string APIs
  ...
  [^1]: C and C++ compile your program in translation units (TUs); roughly, an
  atomic unit as far as the linker is concerned. Usually, it's accurate enough
  to think of a TU as a C file. The linker doesn't have to link functions inside
  the same C file, but it does have to link functions that live in a different C
  file, or a library.

  sp.h does not, of course, have a C file. And yet, since we'd like to use it by
  compiling it alongside our own program, it needs to go *somewhere*. There must
  be *some* TU which has all its symbols.

  This preprocessor guard lets you "turn on" the C file (the implementation) and
  jam it into an actual C file. You can even make an sp.c in your project which
  solely does this; or, you could compile sp.h into a genuine static or shared
  library.
...
#define SP_ZERO_RETURN(t)                                                      \
  {                                                                            \
    t __SP_ZERO_RETURN = SP_ZERO_STRUCT(t);                                    \
    return __dn_zero_return;                                                   \
  }
...
#define SP_FATAL(FMT, ...)                                                     \
  do {                                                                         \
    ...
    SP_LOG("{:color red}: {}", SP_FMT_CSTR("SP_FATAL()"),                      \
           SP_FMT_STR(message));                                               \
    SP_EXIT_FAILURE();                                                         \
  } while (0)
...
#define SP_UNREACHABLE_CASE()                                                  \
  SP_ASSERT(false);                                                            \
  break;
#define sp_unreachable_case() SP_UNREACHABLE_CASE()
#define SP_UNREACHABLE_RETURN(v)                                               \
  SP_ASSERT(false);                                                            \
  return (v)
...
#define SP_ASSERT_FMT(COND, FMT, ...)                                          \
  do {                                                                         \
    if (!(COND)) {                                                             \
      ...
      sp_str_t message = sp_format((FMT), ##__VA_ARGS__);                      \
      SP_LOG("SP_ASSERT({:color red}): {}", SP_FMT_CSTR(condition),            \
             SP_FMT_STR(message));                                             \
      SP_EXIT_FAILURE();                                                       \
    }                                                                          \
  } while (0)
...
#define SP_TYPEDEF_FN(return_type, name, ...)                                  \
  typedef return_type (*name)(__VA_ARGS__)
#define sp_typedef_fn(return_type, name, ...)                                  \
  SP_TYPEDEF_FN(return_type, name, __VA_ARGS__)
...
#define SP_COLOR_RGB(RED, GREEN, BLUE)                                         \
  {.r = (RED) / 255.f, .g = (GREEN) / 255.f, .b = (BLUE) / 255.f, .a = 1.0}
...
#define SP_X_ENUM_CASE_TO_CSTR(ID)                                             \
  case ID: {                                                                   \
    return SP_MACRO_STR(ID);                                                   \
  }
#define SP_X_ENUM_CASE_TO_STRING(ID)                                           \
  case ID: {                                                                   \
    return SP_LIT(SP_MACRO_STR(ID));                                           \
  }
#define SP_X_ENUM_CASE_TO_STRING_UPPER(ID)                                     \
  case ID: {                                                                   \
    return sp_str_to_upper(SP_LIT(SP_MACRO_STR(ID)));                          \
  }
#define SP_X_ENUM_CASE_TO_STRING_LOWER(ID)                                     \
  case ID: {                                                                   \
    return sp_str_to_lower(SP_LIT(SP_MACRO_STR(ID)));                          \
  }
#define SP_X_ENUM_DEFINE(ID) ID,

#define SP_X_NAMED_ENUM_CASE_TO_CSTR(ID, NAME)                                 \
  case ID: {                                                                   \
    return (NAME);                                                             \
  }
#define SP_X_NAMED_ENUM_CASE_TO_STRING(ID, NAME)                               \
  case ID: {                                                                   \
    return sp_str_lit(NAME);                                                   \
  }
#define SP_X_NAMED_ENUM_CASE_TO_STRING_UPPER(ID, NAME)                         \
  case ID: {                                                                   \
    return sp_str_to_upper(sp_str_lit(NAME));                                  \
  }
#define SP_X_NAMED_ENUM_CASE_TO_STRING_LOWER(ID, NAME)                         \
  case ID: {                                                                   \
    return sp_str_to_lower(sp_str_lit(NAME));                                  \
  }
#define SP_X_NAMED_ENUM_DEFINE(ID, NAME) ID,
#define SP_X_NAMED_ENUM_STR_TO_ENUM(ID, NAME)                                  \
  if (sp_str_equal(str, SP_LIT(NAME)))                                         \
    return ID;
...
#define SP_LIMIT_EPOCH_MAX                                                     \
  SP_RVAL(sp_tm_epoch_t) { .s = SP_LIMIT_U64_MAX, .ns = SP_LIMIT_U32_MAX }
...
#define SP_ENTRY(fn)                                                           \
  s32 main(s32 num_args, const c8** args) { return fn(num_args, args); }
#endif
...
// ╚═╝     ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝
// @math
// Totally ripped from Handmade Math and tweaked to follow sp.h's style
// conventions. It's public domain, but here's the credits section from the
// version I ripped. Any usefulness of any of the vector code in this or derived
...
SP_TYPEDEF_FN(void*, sp_allocator_fn_t, void* user_data,
              sp_mem_alloc_mode_t mode, u32 size, void* ptr);
...
SP_API void* sp_mem_allocator_realloc(sp_allocator_t arena, void* ptr,
                                      u32 size);
...
SP_API sp_mem_arena_t*
sp_mem_arena_new_ex(u32 block_size, sp_mem_arena_mode_t mode, u8 alignment);
...
SP_API void* sp_mem_arena_on_alloc(void* ptr, sp_mem_alloc_mode_t mode, u32 n,
                                   void* old);
...
SP_API void* sp_mem_libc_on_alloc(void* ud, sp_mem_alloc_mode_t mode, u32 size,
                                  void* ptr);
...
#define sp_mem_allocator_alloc_type(a, T)                                      \
  (T*)sp_mem_allocator_alloc(a, sizeof(T))
#define sp_mem_allocator_alloc_n(a, T, n)                                      \
  (T*)sp_mem_allocator_alloc(a, (n) * sizeof(T))
...
#define sp_fixed_array_for(arr, it, t)                                         \
  for (t* it = (t*)arr.data; (it - (t*)arr.data) < arr.size; it++)
#define SP_FIXED_ARRAY_RUNTIME_SIZE

SP_API void sp_fixed_array_init(sp_fixed_array_t* fixed_array, u32 capacity,
                                u32 element_size);
SP_API u8* sp_fixed_array_push(sp_fixed_array_t* fixed_array, void* data,
                               u32 count);
...
#define sp_dyn_array_for(__ARR, __IT)                                          \
  for (u32 __IT = 0; __IT < sp_dyn_array_size((__ARR)); __IT++)
#define sp_dyn_array_rfor(__ARR, __IT)                                         \
  for (u32 __IT = sp_dyn_array_size(__ARR); __IT-- > 0;)
...
#define sp_dyn_array_empty(__ARR) (sp_dyn_array_size(__ARR) == 0)
...
#define sp_dyn_array_need_grow(__ARR, __N)                                     \
  ((__ARR) == 0 ||                                                             \
   sp_dyn_array_size(__ARR) + (__N) >= sp_dyn_array_capacity(__ARR))

#define sp_dyn_array_grow(__ARR)                                               \
  sp_dyn_array_resize_impl(                                                    \
    (__ARR), sizeof(*(__ARR)),                                                 \
    sp_dyn_array_capacity(__ARR) ? sp_dyn_array_capacity(__ARR) * 2 : 1)
...
#define sp_dyn_array_reserve(__ARR, __AMOUNT)                                  \
  do {                                                                         \
    ...
    if ((!__ARR) || (u32)__AMOUNT > sp_dyn_array_capacity(__ARR)) {            \
      *((void**)&(__ARR)) =                                                    \
        sp_dyn_array_resize_impl(__ARR, sizeof(*__ARR), __AMOUNT);             \
    }                                                                          \
  } while (0)
...
#define sp_dyn_array_sort(arr, fn)                                             \
  qsort(arr, sp_dyn_array_size(arr), sizeof((arr)[0]), fn)
...
#define sp_rb_empty(__ARR) (sp_rb_size(__ARR) == 0)

#define sp_rb_full(__ARR) (sp_rb_size((__ARR)) == sp_rb_capacity((__ARR)))
    ...
    if ((__ARR) == SP_NULLPTR) {                                               \
      *((void**)&(__ARR)) = sp_rb_grow_impl(SP_NULLPTR, sizeof(*(__ARR)), 8);  \
      ...
      if (sp_rb_mode(__ARR) == SP_RQ_MODE_OVERWRITE) {                         \
        sp_rb_head(__ARR)->head =                                              \
          (sp_rb_head(__ARR)->head + 1) % sp_rb_capacity(__ARR);               \
        sp_rb_head(__ARR)->size--;                                             \
      } else {                                                                 \
        *((void**)&(__ARR)) =                                                  \
          sp_rb_grow_impl(__ARR, sizeof(*(__ARR)), sp_rb_capacity(__ARR) * 2); \
      }                                                                        \
    }                                                                          \
    s32 __sp_rb_tail = (sp_rb_head(__ARR)->head + sp_rb_head(__ARR)->size) %   \
                       sp_rb_capacity(__ARR);                                  \
...
#define sp_rb_for(__ARR, __IT)                                                 \
  for (s32 __IT = 0; __IT < sp_rb_size(__ARR); __IT++)
#define sp_rb_rfor(__ARR, __IT)                                                \
  for (s32 __IT = sp_rb_size(__ARR) - 1; __IT >= 0; __IT--)
...
#define sp_ht_size(ht) ((ht) ? (ht)->size : 0)

#define sp_ht_capacity(ht) ((ht) ? (ht)->capacity : 0)

#define sp_ht_empty(ht) ((ht) ? (ht)->size == 0 : true)
...
#define sp_ht_data_offset_u8(ht, field)                                        \
  (((u8*)(&((ht)->data[0].field))) - sp_ht_data_u8(ht))
#define sp_ht_field_as_u8(ht, field) ((u8*)&((ht)->field))
#define sp_ht_as_u8(ht) ((u8*)(ht))
#define sp_ht_field_offset_u8(ht, field)                                       \
  (sp_ht_field_as_u8(ht, field) - sp_ht_as_u8(ht))
...
#define sp_ht_get_key_n(ht, n) (&(ht)->data[(n)].key)

#define sp_ht_get_n(ht, n) (&(ht)->data[(n)].val)

#define sp_ht_get_tmp_n(ht) sp_ht_get_n(ht, (ht)->info.tmp_idx)

#define sp_ht_getp(ht, key)                                                    \
  (!(ht)                                                                       \
     ? SP_NULLPTR                                                              \
     : ((ht)->tmp_key = (key), (ht)->info.tmp_idx = sp_ht_tmp_key_index(ht),   \
        (ht)->info.tmp_idx == SP_HT_INVALID_INDEX ? SP_NULLPTR                 \
                                                  : sp_ht_get_tmp_n(ht)))

#define sp_ht_key_exists(ht, key)                                              \
  ((ht) &&                                                                     \
   ((ht)->tmp_key = (key), sp_ht_tmp_key_index(ht) != SP_HT_INVALID_INDEX))

#define sp_ht_exists(ht, key) sp_ht_key_exists((ht), (key))
...
#define sp_ht_it_valid(ht, it)                                                 \
  ((ht) && (it) < sp_ht_capacity(ht) &&                                        \
   (ht)->data[(it)].state == SP_HT_ENTRY_ACTIVE)

#define sp_ht_it_advance(ht, it)                                               \
  ((ht) ? sp_ht_it_advance_fn((void**)&(ht)->data, (ht)->capacity, &(it),      \
                              (ht)->info)                                      \
        : (void)0)

#define sp_ht_it_getp(ht, it) (!(ht) ? SP_NULLPTR : sp_ht_get_n(ht, it))

#define sp_ht_it_getkp(ht, it) (!(ht) ? SP_NULLPTR : sp_ht_get_key_n(ht, it))

#define sp_ht_it_init(ht)                                                      \
  (!(ht) ? 0                                                                   \
         : sp_ht_it_init_fn((void**)&(ht)->data, (ht)->capacity, (ht)->info))

#define sp_ht_for(ht, it)                                                      \
  for (sp_ht_it_t it = sp_ht_it_init(ht); sp_ht_it_valid(ht, it);              \
       sp_ht_it_advance(ht, it))
...
#define sp_ht_for_kv(ht, it)                                                   \
  for (struct {                                                                \
         sp_ht_it_t idx;                                                       \
         sp_ht_key_t(ht) key;                                                  \
         sp_ht_value_t(ht) val;                                                \
       } it = {sp_ht_it_init(ht), sp_ht_it_getkp(ht, sp_ht_it_init(ht)),       \
               sp_ht_it_getp(ht, sp_ht_it_init(ht))};                          \
       sp_ht_it_valid(ht, it.idx);                                             \
       (sp_ht_it_advance(ht, it.idx), it.key = sp_ht_it_getkp(ht, it.idx),     \
                                      it.val = sp_ht_it_getp(ht, it.idx)))

#define sp_ht_front(ht)                                                        \
  (!(ht) || !sp_ht_it_valid(ht, sp_ht_it_init(ht))                             \
     ? SP_NULLPTR                                                              \
     : sp_ht_it_getp(ht, sp_ht_it_init(ht)))

#define sp_ht_tmp_key_index(ht)                                                \
  sp_ht_get_key_index_fn((void**)&(ht)->data, (void*)&(ht)->tmp_key,           \
                         (ht)->capacity, (ht)->info)
...
SP_API u64 sp_ht_get_key_index_fn(void** data, void* key, u64 capacity,
                                  sp_ht_info_t info);
SP_API void sp_ht_resize_impl(void** data, u64 old_cap, u64 new_cap,
                              sp_ht_info_t info);
SP_API void sp_ht_insert_impl(void* ht, void* key, void* val,
                              sp_ht_info_t info);
SP_API sp_ht_it_t sp_ht_it_init_fn(void** data, u64 capacity,
                                   sp_ht_info_t info);
SP_API void sp_ht_it_advance_fn(void** data, u64 capacity, u64* it,
                                sp_ht_info_t info);
...
#define sp_str(STR, LEN)                                                       \
  sp_rval(sp_str_t) { .data = (const c8*)(STR), .len = (u32)(LEN) }
...
#define sp_str_for_it(str, it)                                                 \
  for (sp_str_it_t it = sp_str_it(str); sp_str_it_valid(&it);                  \
       sp_str_it_next(&it))
#define sp_str_for_utf8(str, it)                                               \
  for (sp_utf8_it_t it = sp_utf8_it(str); sp_utf8_it_valid(&it);               \
       sp_utf8_it_next(&it))
#define sp_str_rfor_utf8(str, it)                                              \
  for (sp_utf8_it_t it = sp_utf8_rit(str); sp_utf8_it_valid(&it);              \
       sp_utf8_it_prev(&it))
...
SP_API sp_str_t sp_str_join_cstr_n(const c8** strings, u32 num_strings,
                                   sp_str_t join);
...
SP_API sp_str_t sp_str_reduce(sp_str_t* strs, u32 n, void* ud,
                              sp_str_reduce_fn_t fn);
...
SP_API sp_da(sp_str_t)
  sp_str_map(sp_str_t* s, u32 n, void* ud, sp_str_map_fn_t fn);
...
SP_API void sp_str_builder_append_utf8(sp_str_builder_t* builder,
                                       u32 codepoint);
...
SP_API void sp_str_builder_append_cstr(sp_str_builder_t* builder,
                                       const c8* str);
SP_API void sp_str_builder_append_c8(sp_str_builder_t* builder, c8 c);
SP_API void sp_str_builder_append_fmt_str(sp_str_builder_t* builder,
                                          sp_str_t fmt, ...);
...
#define sp_mem_slice_for_it(slice, it)                                         \
  for (sp_mem_slice_it_t it = sp_mem_slice_it(slice);                          \
       sp_mem_slice_it_valid(&it); sp_mem_slice_it_next(&it))

SP_API sp_mem_slice_t sp_mem_slice(u8* ptr, u64 len);
SP_API sp_mem_slice_t sp_mem_slice_copy(sp_mem_slice_t slice);
SP_API sp_mem_slice_t sp_mem_slice_sub(sp_mem_slice_t slice, u64 start,
                                       u64 len);
...
SP_API bool sp_atomic_s32_cmp_and_swap(sp_atomic_s32* value, s32 current,
                                       s32 desired);
...
SP_API void sp_thread_init(sp_thread_t* thread, sp_thread_fn_t fn,
                           void* userdata);
...
SP_TYPEDEF_FN(u64, sp_io_reader_read_cb, sp_io_reader_t* r, void* ptr,
              u64 size);
SP_TYPEDEF_FN(s64, sp_io_reader_seek_cb, sp_io_reader_t* r, s64 offset,
              sp_io_whence_t whence);
SP_TYPEDEF_FN(u64, sp_io_reader_size_cb, sp_io_reader_t* r);
SP_TYPEDEF_FN(void, sp_io_reader_close_cb, sp_io_reader_t* r);

SP_TYPEDEF_FN(u64, sp_io_writer_write_cb, sp_io_writer_t* w, const void* ptr,
              u64 size);
SP_TYPEDEF_FN(s64, sp_io_writer_seek_cb, sp_io_writer_t* w, s64 offset,
              sp_io_whence_t whence);
...
SP_API s64 sp_io_reader_seek(sp_io_reader_t* reader, s64 offset,
                             sp_io_whence_t whence);
...
SP_API sp_io_reader_t sp_io_reader_from_fd(sp_os_file_handle_t fd,
                                           sp_io_close_mode_t mode);
SP_API sp_io_reader_t sp_io_reader_from_mem(const void* ptr, u64 size);
SP_API void sp_io_reader_set_buffer(sp_io_reader_t* reader, u8* buf,
                                    u64 capacity);
...
SP_API s64 sp_io_writer_seek(sp_io_writer_t* writer, s64 offset,
                             sp_io_whence_t whence);
SP_API u64 sp_io_writer_size(sp_io_writer_t* w);
SP_API void sp_io_writer_close(sp_io_writer_t* w);
SP_API sp_io_writer_t sp_io_writer_from_file(sp_str_t path,
                                             sp_io_write_mode_t mode);
SP_API sp_io_writer_t sp_io_writer_from_fd(s32 fd,
                                           sp_io_close_mode_t close_mode);
SP_API sp_io_writer_t sp_io_writer_from_mem(void* ptr, u64 size);
SP_API sp_io_writer_t sp_io_writer_from_dyn_mem();
SP_API sp_io_writer_t sp_io_writer_from_dyn_mem_ex(u8* buffer, u64 size,
                                                   sp_allocator_t allocator);
SP_API void sp_io_writer_set_buffer(sp_io_writer_t* writer, u8* buf,
                                    u64 capacity);
...
typedef enum { SP_PS_STATE_RUNNING, SP_PS_STATE_DONE } sp_ps_state_t;

#define SP_PS_NO_STDIO                                                         \
  (sp_ps_io_config_t) {                                                        \
    .in = {.mode = SP_PS_IO_NULL}, .out = {.mode = SP_PS_IO_NULL},             \
...
#define SP_FMT_ARG(T, V)                                                       \
  SP_RVAL(sp_format_arg_t) { .SP_FMT_UNION(T) = (V), .id = SP_FMT_ID(T) }
...
#undef SP_FMT_X
#define SP_FMT_X(name, type)                                                   \
  void sp_fmt_format_##name(sp_str_builder_t* builder, sp_format_arg_t* buffer);
SP_FORMAT_TYPES
...
SP_API void sp_fmon_init(sp_fmon_t* m, sp_fmon_fn_t fn,
                         sp_fmon_event_kind_t events, void* user_data);
SP_API void sp_fmon_init_ex(sp_fmon_t* m, sp_fmon_fn_t fn,
                            sp_fmon_event_kind_t events, void* user_data,
                            u32 debounce, sp_allocator_t alloc);
...
SP_API sp_fmon_cache_t* sp_fmon_get_or_insert_cache(sp_fmon_t* monitor,
                                                    sp_str_t file_path);
#endif
...
SP_TYPEDEF_FN(void, sp_asset_completion_fn_t,
...
#define sp_asset_import_context_get_asset(ctx)                                 \
  (&(ctx)->registry->assets[(ctx)->asset_index])
...
SP_API void sp_asset_registry_init(sp_asset_registry_t* r,
                                   sp_asset_registry_config_t config);
SP_API void sp_asset_registry_shutdown(sp_asset_registry_t* r);
SP_API sp_future_t* sp_asset_registry_import(sp_asset_registry_t* r,
                                             sp_asset_kind_t k, sp_str_t name,
                                             void* user_data);
SP_API sp_asset_t* sp_asset_registry_add(sp_asset_registry_t* r,
                                         sp_asset_kind_t k, sp_str_t name,
                                         void* user_data);
SP_API sp_asset_t* sp_asset_registry_find(sp_asset_registry_t* r,
                                          sp_asset_kind_t kind, sp_str_t name);
...
SP_API sp_asset_importer_t*
sp_asset_registry_find_importer(sp_asset_registry_t* r, sp_asset_kind_t kind);
...
sp_elf_section_t* sp_elf_add_section(sp_elf_t* elf, sp_str_t name, u32 type,
                                     u64 addralign);
...
void* sp_elf_section_reserve_aligned(sp_elf_section_t* section, u64 size,
                                     u64 align);
...
u32 sp_elf_add_symbol(sp_elf_section_t* symbols, sp_elf_t* elf, sp_str_t name,
                      u64 value, u64 size, u8 bind, u8 type, u16 shndx);
...
u32 sp_macho_add_symbol(sp_macho_t* m, sp_str_t name, u32 sect, u64 offset,
                        sp_macho_sym_bind_t bind);
...
#if !defined(SP_FREESTANDING)
sp_rt_t sp_rt = {.mutex = PTHREAD_MUTEX_INITIALIZER,
...
void* sp_sys_memmove(void* dest, const void* src, size_t n) {
  ...
  __asm__ __volatile__("lea -1(%[dest], %[n]), %%rdi\n\t"
                       "lea -1(%[src], %[n]), %%rsi\n\t"
                       "mov %[n], %%rcx\n\t"
                       "std\n\t"
                       "rep movsb\n\t"
...
void* sp_sys_memcpy(void* dest, const void* src, size_t n) {
  ...
  u64* d64;
  const u64* s64;
  __asm__ __volatile__("ldp x4, x5, [%[src]]\n\t"
                       "stp x4, x5, [%[dst]]"
                       :
                       : [src] "r"(s), [dst] "r"(d)
                       : "x4", "x5", "memory");
...
void* memset(void* dest, s32 c, u64 n) { return sp_sys_memset(dest, c, n); }
...
void* sp_sys_mmap(void* addr, u64 len, s32 prot, s32 flags, s32 fd,
                  s64 offset) {
  return (void*)sp_syscall6(SP_SYSCALL_NUM_MMAP, (s64)addr, (s64)len, prot,
                            flags, fd, offset);
}
...
void* sp_sys_mremap(void* old_addr, u64 old_size, u64 new_size, s32 flags) {
  return (void*)sp_syscall4(SP_SYSCALL_NUM_MREMAP, (s64)old_addr, (s64)old_size,
                            (s64)new_size, flags);
}
...
void* sp_sys_alloc(u64 n) {
  ...
  void* p = sp_sys_mmap(0, total, SP_PROT_READ | SP_PROT_WRITE,
...
void* sp_sys_alloc_zero(u64 n) { return sp_sys_alloc(n); }
...
#else
  return (s32)sp_syscall4(SP_SYSCALL_NUM_OPENAT, SP_AT_FDCWD, (s64)path, flags,
                          mode);
#endif
}
...
s32 sp_sys_close(s32 fd) { return (s32)sp_syscall1(SP_SYSCALL_NUM_CLOSE, fd); }
...
#else
  return (s32)sp_syscall4(SP_SYSCALL_NUM_NEWFSTATAT, SP_AT_FDCWD, (s64)path,
                          (s64)st, 0);
#endif
}
...
#else
  return (s32)sp_syscall3(SP_SYSCALL_NUM_UNLINKAT, SP_AT_FDCWD, (s64)path,
                          SP_AT_REMOVEDIR);
#endif
}
...
#else
  return (s32)sp_syscall4(SP_SYSCALL_NUM_RENAMEAT, SP_AT_FDCWD, (s64)oldpath,
                          SP_AT_FDCWD, (s64)newpath);
#endif
}
...
#else
  return sp_syscall4(SP_SYSCALL_NUM_READLINKAT, SP_AT_FDCWD, (s64)path,
                     (s64)buf, (s64)size);
#endif
}
...
#else
  return (s32)sp_syscall3(SP_SYSCALL_NUM_SYMLINKAT, (s64)target, SP_AT_FDCWD,
                          (s64)linkpath);
#endif
}
...
#else
  return (s32)sp_syscall5(SP_SYSCALL_NUM_LINKAT, SP_AT_FDCWD, (s64)oldpath,
                          SP_AT_FDCWD, (s64)newpath, 0);
#endif
}
...
#else
  return (s32)sp_syscall4(SP_SYSCALL_NUM_FCHMODAT, SP_AT_FDCWD, (s64)path, mode,
                          0);
#endif
}
...
s32 sp_sys_getpid(void) { return (s32)sp_syscall0(SP_SYSCALL_NUM_GETPID); }
...
s32 sp_sys_inotify_add_watch(s32 fd, const c8* pathname, u32 mask) {
  return (s32)sp_syscall3(SP_SYSCALL_NUM_INOTIFY_ADD_WATCH, fd, (s64)pathname,
                          mask);
}
...
#elif defined(SP_ARM64)
  sp_sys_timespec_t ts = {.tv_sec = timeout / 1000,
                          .tv_nsec = (timeout % 1000) * 1000000};
  sp_sys_timespec_t* tsp = timeout < 0 ? SP_NULLPTR : &ts;
  return (s32)sp_syscall5(SP_SYSCALL_NUM_PPOLL, (s64)fds, (s64)nfds, (s64)tsp,
                          0, 0);
#endif
}

s32 sp_sys_wait4(s32 pid, s32* status, s32 options, void* rusage) {
  return (s32)sp_syscall4(SP_SYSCALL_NUM_WAIT4, pid, (s64)status, options,
                          (s64)rusage);
}
...
sp_color_t sp_color_rgb_to_hsv(sp_color_t color) {
  ...
  return (sp_color_t){
    .h = h * 360.0f, .s = s * 100.0f, .v = v * 100.0f, .a = color.a};
}
...
sp_color_t sp_color_hsv_to_rgb(sp_color_t color) {
  ...
  return (sp_color_t){.r = r, .g = g, .b = b, .a = color.a};
}

f32 sp_inv_sqrtf(f32 value) { return 1.0f / sp_sqrtf(value); }

f32 sp_lerp(f32 a, f32 t, f32 b) { return (1.0f - t) * a + t * b; }
...
sp_vec2_t sp_vec2(f32 x, f32 y) { return (sp_vec2_t){.x = x, .y = y}; }
...
sp_vec4_t sp_vec4_add(sp_vec4_t left, sp_vec4_t right) {
  return (sp_vec4_t){.x = left.x + right.x,
                     .y = left.y + right.y,
                     .z = left.z + right.z,
                     .w = left.w + right.w};
}
...
sp_vec4_t sp_vec4_scale(sp_vec4_t left, f32 right) {
  return (sp_vec4_t){.x = left.x * right,
                     .y = left.y * right,
                     .z = left.z * right,
                     .w = left.w * right};
}
...
sp_vec3_t sp_vec3_cross(sp_vec3_t left, sp_vec3_t right) {
  return (sp_vec3_t){.x = (left.y * right.z) - (left.z * right.y),
                     .y = (left.z * right.x) - (left.x * right.z),
                     .z = (left.x * right.y) - (left.y * right.x)};
}

f32 sp_vec2_len_sqr(sp_vec2_t v) { return sp_vec2_dot(v, v); }

f32 sp_vec3_len_sqr(sp_vec3_t v) { return sp_vec3_dot(v, v); }

f32 sp_vec4_len_sqr(sp_vec4_t v) { return sp_vec4_dot(v, v); }

f32 sp_vec2_len(sp_vec2_t v) { return sp_sqrtf(sp_vec2_len_sqr(v)); }

f32 sp_vec3_len(sp_vec3_t v) { return sp_sqrtf(sp_vec3_len_sqr(v)); }

f32 sp_vec4_len(sp_vec4_t v) { return sp_sqrtf(sp_vec4_len_sqr(v)); }
...
sp_interp_t sp_interp_build(f32 start, f32 target, f32 time) {
  return (sp_interp_t){
    .start = start, .delta = target - start, .t = 0, .time_scale = 1.0f / time};
}
...
f32 sp_interp_ease_inout(sp_interp_t* interp) {
  ...
  if (interp->t < 0.5f) {
    ...
    eased =
      1.0f - (-2.0f * interp->t + 2.0f) * (-2.0f * interp->t + 2.0f) / 2.0f;
  }
  return interp->start + interp->delta * eased;
}
...
#define sp_rotate_left(__V, __N)                                               \
  (((__V) << (__N)) | ((__V) >> (SP_SIZE_T_BITS - (__N))))
#define sp_rotate_right(__V, __N)                                              \
  (((__V) >> (__N)) | ((__V) << (SP_SIZE_T_BITS - (__N))))
...
#define sp_sipround()                                                          \
  do {                                                                         \
    v0 += v1;                                                                  \
    v1 = sp_rotate_left(v1, 13);                                               \
    v1 ^= v0;                                                                  \
    v0 = sp_rotate_left(v0, SP_SIZE_T_BITS / 2);                               \
    v2 += v3;                                                                  \
    v3 = sp_rotate_left(v3, 16);                                               \
    v3 ^= v2;                                                                  \
    v2 += v1;                                                                  \
    v1 = sp_rotate_left(v1, 17);                                               \
    v1 ^= v2;                                                                  \
    v2 = sp_rotate_left(v2, SP_SIZE_T_BITS / 2);                               \
    v0 += v3;                                                                  \
    v3 = sp_rotate_left(v3, 21);                                               \
    v3 ^= v0;                                                                  \
  } while (0)
...
u64 sp_ht_get_key_index_fn(void** data, void* key, u64 capacity,
                           sp_ht_info_t info) {
  if (!data || !*data || !key || !capacity)
  ...
  for (u64 c = 0; c < capacity; ++c) {
    ...
    sp_ht_entry_state state =
      *(sp_ht_entry_state*)((c8*)(*data) + offset + info.stride.kv);
...
void sp_ht_resize_impl(void** data, u64 old_cap, u64 new_cap,
                       sp_ht_info_t info) {
  if (!data || new_cap <= old_cap)
    return;
  ...
  for (u64 i = 0; i < old_cap; ++i) {
    u64 offset = i * info.stride.entry;
    sp_ht_entry_state state =
      *(sp_ht_entry_state*)((c8*)old_data + offset + info.stride.kv);
    ...
    while (*(sp_ht_entry_state*)((c8*)new_data + new_idx * info.stride.entry +
                                 info.stride.kv) == SP_HT_ENTRY_ACTIVE) {
      new_idx = (new_idx + 1) % new_cap;
    }

    sp_mem_copy((c8*)old_data + offset,
                (c8*)new_data + new_idx * info.stride.entry, info.stride.kv);
    *(sp_ht_entry_state*)((c8*)new_data + new_idx * info.stride.entry +
                          info.stride.kv) = SP_HT_ENTRY_ACTIVE;
  }
...
void sp_ht_insert_impl(void* ht, void* key, void* val, sp_ht_info_t info) {
  ...
  if (existing != SP_HT_INVALID_INDEX) {
    ...
    sp_hash_t hash = info.fn.hash(key, info.size.key);
    u64 idx = hash % cap;
    while (*(sp_ht_entry_state*)((u8*)(*data) + idx * info.stride.entry +
                                 info.stride.kv) == SP_HT_ENTRY_ACTIVE) {
      idx = (idx + 1) % cap;
    }
...
sp_ht_it_t sp_ht_it_init_fn(void** data, u64 capacity, sp_ht_info_t info) {
  ...
  for (; it < capacity; ++it) {
    ...
    sp_ht_entry_state state =
      *(sp_ht_entry_state*)((u8*)*data + offset + info.stride.kv);
    if (state == SP_HT_ENTRY_ACTIVE) {
      break;
    }
  }
...
void sp_ht_it_advance_fn(void** data, u64 capacity, u64* it,
                         sp_ht_info_t info) {
  if (!data || !*data)
    return;
  (*it)++;
...
void* sp_dyn_array_resize_impl(void* arr, u32 sz, u32 amount) {
  ...
  sp_dyn_array* data = (sp_dyn_array*)sp_realloc(
    arr ? sp_dyn_array_head(arr) : 0, capacity * sz + sizeof(sp_dyn_array));
...
void** sp_dyn_array_init(void** arr, u32 val_len) {
  if (*arr == NULL) {
    sp_dyn_array* data =
      (sp_dyn_array*)sp_alloc(val_len + sizeof(sp_dyn_array));
    data->size = 0;
    data->capacity = 1;
    *arr = ((s32*)data + 2);
  }
...
void sp_dyn_array_push_f(void** arr, void* val, u32 val_len) {
  ...
  if (*arr) {
    sp_mem_copy(val, ((u8*)(*arr)) + sp_dyn_array_size(*arr) * val_len,
                val_len);
...
void* sp_rb_grow_impl(void* arr, u32 elem_size, u32 new_cap) {
  sp_ring_buffer_t* new_data =
    (sp_ring_buffer_t*)sp_alloc(new_cap * elem_size + sizeof(sp_ring_buffer_t));
  if (!new_data)
    return SP_NULLPTR;
  ...
  if (arr) {
    ...
    if (first_chunk > old_size)
      first_chunk = old_size;
    sp_mem_copy(old_arr + old_head * elem_size, new_arr,
                first_chunk * elem_size);

    s32 second_chunk = old_size - first_chunk;
    if (second_chunk > 0) {
      sp_mem_copy(old_arr, new_arr + first_chunk * elem_size,
...
// ╚═╝
// @fixed_array
void sp_fixed_array_init(sp_fixed_array_t* buffer, u32 max_vertices,
                         u32 element_size) {
  SP_ASSERT(buffer);
...
u8* sp_fixed_array_push(sp_fixed_array_t* buffer, void* data, u32 count) {
  ...
  if (data)
    sp_mem_copy(data, reserved, buffer->element_size * count);
  return reserved;
}
...
bool sp_parse_s64_ex(sp_str_t str, s64* out) {
  ...
  u64 abs_value;
  if (!sp_parse_u64_ex(num_str, &abs_value))
    return false;

  if (negative) {
    if (abs_value > (u64)INT64_MAX + 1)
      return false; // overflow
    *out = -(s64)abs_value;
  } else {
    if (abs_value > INT64_MAX)
      return false; // overflow
    *out = (s64)abs_value;
  }
...
bool sp_parse_is_digit(c8 c) { return c >= '0' && c <= '9'; }
...
void sp_fmt_format_unsigned(sp_str_builder_t* builder, u64 num,
                            u32 max_digits) {
  SP_ASSERT(builder);
...
void sp_fmt_format_hex(sp_str_builder_t* builder, u64 value, u32 min_width,
                       const c8* prefix) {
  SP_ASSERT(builder);
  ...
  if (value == 0) {
    ...
    sp_for(i, zero_count) { sp_str_builder_append_c8(builder, '0'); }
    return;
  }
...
void sp_fmt_format_str_builder(sp_str_builder_t* builder,
                               sp_format_arg_t* arg) {
...
void sp_fmt_format_fixed_array(sp_str_builder_t* builder,
                               sp_format_arg_t* arg) {
...
void sp_format_parser_eat(sp_format_parser_t* parser) { parser->it++; }
...
sp_format_specifier_flag_t sp_format_specifier_flag_from_str(sp_str_t id) {
  if (sp_str_equal_cstr(id, "color"))
    return SP_FORMAT_SPECIFIER_FLAG_FG_COLOR;
  if (sp_str_equal_cstr(id, "fg"))
    return SP_FORMAT_SPECIFIER_FLAG_FG_COLOR;
  if (sp_str_equal_cstr(id, "bg"))
    return SP_FORMAT_SPECIFIER_FLAG_BG_COLOR;
  if (sp_str_equal_cstr(id, "pad"))
    return SP_FORMAT_SPECIFIER_FLAG_PAD;
  return SP_FORMAT_SPECIFIER_FLAG_NONE;
}

sp_str_t sp_format_color_id_to_ansi_fg(sp_str_t id) {
  if (sp_str_equal_cstr(id, "black"))
    return SP_CSTR(SP_ANSI_FG_BLACK);
  if (sp_str_equal_cstr(id, "red"))
    return SP_CSTR(SP_ANSI_FG_RED);
  if (sp_str_equal_cstr(id, "green"))
    return SP_CSTR(SP_ANSI_FG_GREEN);
  if (sp_str_equal_cstr(id, "yellow"))
    return SP_CSTR(SP_ANSI_FG_YELLOW);
  if (sp_str_equal_cstr(id, "blue"))
    return SP_CSTR(SP_ANSI_FG_BLUE);
  if (sp_str_equal_cstr(id, "magenta"))
    return SP_CSTR(SP_ANSI_FG_MAGENTA);
  if (sp_str_equal_cstr(id, "cyan"))
    return SP_CSTR(SP_ANSI_FG_CYAN);
  if (sp_str_equal_cstr(id, "white"))
    return SP_CSTR(SP_ANSI_FG_WHITE);
  if (sp_str_equal_cstr(id, "brightblack"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_BLACK);
  if (sp_str_equal_cstr(id, "brightred"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_RED);
  if (sp_str_equal_cstr(id, "brightgreen"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_GREEN);
  if (sp_str_equal_cstr(id, "brightyellow"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_YELLOW);
  if (sp_str_equal_cstr(id, "brightblue"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_BLUE);
  if (sp_str_equal_cstr(id, "brightmagenta"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_MAGENTA);
  if (sp_str_equal_cstr(id, "brightcyan"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_CYAN);
  if (sp_str_equal_cstr(id, "brightwhite"))
    return SP_CSTR(SP_ANSI_FG_BRIGHT_WHITE);
...
#define SP_FMT_X(ID, t)                                                        \
  (sp_formatter_t){.fn = SP_FMT_FN(ID), .id = SP_FMT_ID(ID)},

  static sp_formatter_t formatters[] = {SP_FORMAT_TYPES};

  sp_mem_scratch_t scratch = sp_mem_begin_scratch();

  sp_str_builder_t builder = SP_ZERO_INITIALIZE();
  sp_format_parser_t parser = {.fmt = fmt, .it = 0};
  ...
  while (true) {
    ...
    case '{': {
      ...
      u64 formatted_value_len =
        sp_io_writer_size(builder.writer) - formatted_value_start;
      ...
      if (specifier.flags & SP_FORMAT_SPECIFIER_FLAG_PAD) {
        if (formatted_value_len < specifier.pad) {
          ...
          sp_for(i, spaces_needed) { sp_str_builder_append_c8(&builder, ' '); }
        }
      }
...
void sp_context_pop() {
  sp_tls_rt_t* state = sp_tls_rt_get();
  SP_ASSERT(state->index > 0);
  state->contexts[state->index] =
    SP_ZERO_STRUCT(sp_context_t); // Not required, just for the debugger
  state->index--;
}
...
sp_tls_rt_t* sp_tls_rt_get() {
  ...
  if (!state) {
    ...
    *state = (sp_tls_rt_t){
      .contexts = {{.allocator = sp_mem_libc_new()}},
      .index = 0,
    };
...
void* sp_mem_allocator_alloc(sp_allocator_t allocator, u32 size) {
  return allocator.on_alloc(allocator.user_data, SP_ALLOCATOR_MODE_ALLOC, size,
                            NULL);
}

void* sp_mem_allocator_realloc(sp_allocator_t allocator, void* memory,
                               u32 size) {
  return allocator.on_alloc(allocator.user_data, SP_ALLOCATOR_MODE_RESIZE, size,
                            memory);
}
...
sp_mem_arena_block_t* sp_mem_arena_block_new(sp_mem_arena_t* arena,
                                             u32 capacity) {
  sp_mem_arena_block_t* block =
    sp_mem_allocator_alloc_type(arena->allocator, sp_mem_arena_block_t);
...
sp_allocator_t sp_mem_arena_as_allocator(sp_mem_arena_t* a) {
  return (sp_allocator_t){.on_alloc = sp_mem_arena_on_alloc, .user_data = a};
}
...
sp_mem_arena_block_t* sp_mem_arena_get_block(sp_mem_arena_t* arena,
                                             u32 alloc_size) {
  ...
  if (block->next && block->next->capacity >= alloc_size) {
    ...
    sp_mem_arena_block_t* new_block =
      sp_mem_arena_block_new(arena, new_capacity);
...
void* sp_mem_arena_alloc_with_header(sp_mem_arena_t* arena, u32 size) {
  ...
  sp_mem_arena_header_t* header =
    (sp_mem_arena_header_t*)sp_mem_arena_align_block(block, arena->alignment);
  header->size = size;
...
void* sp_mem_arena_on_alloc(void* user_data, sp_mem_alloc_mode_t mode, u32 size,
                            void* old_memory) {
...
void* sp_mem_arena_alloc(sp_mem_arena_t* arena, u32 size) {
  return sp_mem_arena_on_alloc(arena, SP_ALLOCATOR_MODE_ALLOC, size,
                               SP_NULLPTR);
}
...
sp_mem_arena_marker_t sp_mem_arena_mark(sp_mem_arena_t* a) {
  return (sp_mem_arena_marker_t){
    .arena = a, .block = a->current, .mark = a->current->bytes_used};
}
...
sp_mem_scratch_t sp_mem_begin_scratch() {
  ...
  sp_mem_scratch_t scratch = {.marker = sp_mem_arena_mark(runtime->scratch),
...
void* sp_mem_libc_on_alloc(void* user_data, sp_mem_alloc_mode_t mode, u32 size,
                           void* ptr) {
  switch (mode) {
  case SP_ALLOCATOR_MODE_ALLOC: {
    u32 total_size = size + sizeof(sp_mem_libc_metadata_t);
    sp_mem_libc_metadata_t* metadata =
      (sp_mem_libc_metadata_t*)sp_mem_os_alloc_zero(total_size);
    metadata->size = size;
    return metadata + 1;
  }
  case SP_ALLOCATOR_MODE_RESIZE: {
    if (!ptr) {
      return sp_mem_libc_on_alloc(user_data, SP_ALLOCATOR_MODE_ALLOC, size,
                                  NULL);
    }
    ...
    void* buffer =
      sp_mem_libc_on_alloc(user_data, SP_ALLOCATOR_MODE_ALLOC, size, NULL);
...
void* sp_mem_os_alloc(u32 size) { return sp_sys_alloc(size); }

void* sp_mem_os_alloc_zero(u32 size) { return sp_sys_alloc_zero(size); }

void* sp_mem_os_realloc(void* ptr, u32 size) {
  return sp_sys_realloc(ptr, size);
}

void sp_mem_os_free(void* ptr) { sp_sys_free(ptr); }
#else
void* sp_mem_os_alloc(u32 size) { return malloc(size); }

void* sp_mem_os_alloc_zero(u32 size) { return calloc(size, 1); }

void* sp_mem_os_realloc(void* ptr, u32 size) { return realloc(ptr, size); }

void sp_mem_os_free(void* ptr) { free(ptr); }
#endif
...
sp_mem_slice_it_t sp_mem_slice_it(sp_mem_slice_t slice) {
  return (sp_mem_slice_it_t){
    .slice = slice, .index = 0, .byte = (u8)(slice.len ? slice.data[0] : 0)};
}
...
sp_str_t sp_mem_buffer_as_str(sp_mem_buffer_t* buffer) {
  return (sp_str_t){.data = (c8*)buffer->data, .len = (u32)buffer->len};
}

c8* sp_mem_buffer_as_cstr(sp_mem_buffer_t* buffer) { return (c8*)buffer->data; }
...
void sp_cstr_copy_to_sized(const c8* str, u32 length, c8* buffer,
                           u32 buffer_length) {
  if (!str)
    return;
  if (!buffer)
...
sp_str_t sp_cstr_as_str(const c8* str) {
  return (sp_str_t){.data = str, .len = sp_cstr_len(str)};
}

sp_str_t sp_cstr_as_str_n(const c8* str, u32 n) {
  return (sp_str_t){.data = str, .len = n};
}
...
c8 sp_c8_to_lower(c8 c) { return (c >= 'A' && c <= 'Z') ? (c + 32) : (c); }

c8 sp_c8_to_upper(c8 c) { return (c >= 'a' && c <= 'z') ? (c - 32) : (c); }

u8 sp_utf8_num_bytes_from_byte(u8 byte) {
  if (byte < SP_UTF8_2_BYTE_MIN)
    return 1;
  if ((byte & 0xE0) == 0xC0)
    return 2;
  if ((byte & 0xF0) == 0xE0)
...
u8 sp_utf8_num_bytes_from_codepoint(u32 codepoint) {
  if (codepoint < SP_UTF8_2_BYTE_MIN)
    return 1;
  if (codepoint < SP_UTF8_3_BYTE_MIN)
    return 2;
  if (codepoint < SP_UTF8_4_BYTE_MIN)
    return 3;
  if (codepoint <= SP_UTF8_MAX)
    return 4;
  return 0;
}

bool sp_utf8_is_cont(u8 b) { return (b & 0xC0) == 0x80; }

bool sp_utf8_is_surrogate(u32 codepoint) {
  return (codepoint >= SP_UTF8_SURROGATE_MIN) &&
         (codepoint <= SP_UTF8_SURROGATE_MAX);
}
...
bool sp_utf8_is_bounds_ok(u32 codepoint, u8 len) {
  ...
  switch (len) {
  case 2: {
    return (codepoint >= SP_UTF8_2_BYTE_MIN);
  }
  case 3: {
    return (codepoint >= SP_UTF8_3_BYTE_MIN);
  }
  case 4: {
    return (codepoint >= SP_UTF8_4_BYTE_MIN);
  }
  }
...
bool sp_utf8_is_byte_ascii(u8 b) { return b < SP_UTF8_ASCII_MAX; }
...
u32 sp_utf8_to_lower(u32 codepoint) {
  if (!sp_utf8_is_codepoint_ascii(codepoint))
    return codepoint;
...
u32 sp_utf8_decode(const u8* ptr) {
  ...
  case 2:
    return sp_utf8_mask(ptr[0], 0x1F, 6) | sp_utf8_mask(ptr[1], 0x3F, 0);
  case 3:
    return sp_utf8_mask(ptr[0], 0x0F, 12) | sp_utf8_mask(ptr[1], 0x3F, 6) |
           sp_utf8_mask(ptr[2], 0x3F, 0);
  case 4:
    return sp_utf8_mask(ptr[0], 0x07, 18) | sp_utf8_mask(ptr[1], 0x3F, 12) |
           sp_utf8_mask(ptr[2], 0x3F, 6) | sp_utf8_mask(ptr[3], 0x3f, 0);
  default:
    return SP_UTF8_REPLACEMENT;
  }
...
sp_utf8_it_t sp_utf8_it(sp_str_t str) {
  if (sp_str_empty(str))
    return sp_zero_struct(sp_utf8_it_t);
...
sp_utf8_it_t sp_utf8_rit(sp_str_t str) {
  ...
  // @spader
  // the cast is objectively wrong here. i just use s32 in the iterator to give
  // me a sentinel when reverse iterating. which in and of itself is stupid.
  // double down.
  sp_utf8_it_t it = {.str = {.data = (const u8*)str.data, .len = str.len},
                     .index = (s32)str.len};
  sp_utf8_it_prev(&it);
  return it;
}
...
bool sp_utf8_it_valid(sp_utf8_it_t* it) {
  if (!it)
    return false;
  return (it->index < (s32)it->str.len) && (it->index >= 0) &&
         (it->codepoint_len);
}
...
void sp_utf8_it_prev(sp_utf8_it_t* it) {
  ...
  while (true) {
    ...
    if (it->index < 0)
      return;
    if (!sp_utf8_is_cont(it->str.data[it->index]))
      break;
  }
...
bool sp_utf8_validate(sp_str_t str) {
  ...
  while (it < str.len) {
    ...
    if (sp_utf8_is_surrogate(codepoint))
      return false;
    if (!sp_utf8_is_bounds_ok(codepoint, len))
      return false;
...
bool sp_str_empty(sp_str_t str) { return !str.len; }
...
bool sp_str_valid(sp_str_t str) { return str.data != NULL; }
...
c8 sp_str_back(sp_str_t str) { return str.data[str.len - 1]; }
...
sp_str_t sp_str_join_cstr_n(const c8** strings, u32 num_strings,
                            sp_str_t join) {
  sp_str_builder_t builder = SP_ZERO_INITIALIZE();
...
sp_str_it_t sp_str_it(sp_str_t str) {
  return (sp_str_it_t){
    .str = str, .index = 0, .c = str.len ? str.data[0] : '\0'};
}

bool sp_str_it_valid(sp_str_it_t* it) { return it->index < it->str.len; }
...
sp_str_t sp_str_reduce(sp_str_t* strings, u32 num_strings, void* user_data,
                       sp_str_reduce_fn_t fn) {
  sp_str_reduce_context_t context = {
    .user_data = user_data,
    .builder = SP_ZERO_INITIALIZE(),
    .elements = {.data = strings, .len = num_strings},
  };
...
sp_str_t sp_str_join_n(sp_str_t* strings, u32 num_strings, sp_str_t joiner) {
  return sp_str_reduce(strings, num_strings, &joiner,
...
sp_dyn_array(sp_str_t) sp_str_map(sp_str_t* strs, u32 num_strs, void* user_data,
                                  sp_str_map_fn_t fn) {
  sp_dyn_array(sp_str_t) results = SP_NULLPTR;

  sp_for(it, num_strs) {
    sp_str_map_context_t context = {.str = strs[it], .user_data = user_data};
...
void sp_str_reduce_kernel_contains(sp_str_reduce_context_t* context) {
  sp_str_contains_context_t* data =
    (sp_str_contains_context_t*)context->user_data;
...
void sp_str_reduce_kernel_count(sp_str_reduce_context_t* context) {
  ...
  if (context->str.len >= data->needle.len) {
    for (u32 i = 0; i <= context->str.len - data->needle.len; i++) {
      if (sp_mem_is_equal(context->str.data + i, data->needle.data,
                          data->needle.len)) {
        data->count++;
...
sp_str_t sp_fs_join_path(sp_str_t a, sp_str_t b) {
  ...
  sp_str_t a_copy = a;
  if (a_copy.len > 0 && (a_copy.data[a_copy.len - 1] == '/' ||
                         a_copy.data[a_copy.len - 1] == '\\')) {
    a_copy.len--;
  }
...
sp_str_t sp_fs_get_ext(sp_str_t path) {
  for (u32 index = 0; index < path.len; index++) {
    ...
    switch (c) {
    case '.':
      return sp_str_sub_reverse(path, 0, index);
    case '/':
...
sp_err_t sp_fs_link(sp_str_t from, sp_str_t to, sp_os_link_kind_t kind) {
  switch (kind) {
  case SP_FS_LINK_HARD:
    return sp_fs_create_hard_link(from, to);
  case SP_FS_LINK_SYMBOLIC:
    return sp_fs_create_sym_link(from, to);
  case SP_FS_LINK_COPY:
...
void sp_os_create_dir(sp_str_t path) {
  ...
  while (normalized.len > 0 && (normalized.data[normalized.len - 1] == '/' ||
                                normalized.data[normalized.len - 1] == '\\')) {
    normalized = sp_str(normalized.data, normalized.len - 1);
  }

  if (sp_os_does_path_exist(normalized))
    return;
...
void sp_fs_create_file(sp_str_t path) {
  sp_win32_handle_t handle =
    CreateFileA(sp_str_to_cstr(path), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
sp_err_t sp_fs_copy(sp_str_t from, sp_str_t to) {
  if (sp_fs_is_glob(from)) {
    ...
    sp_fs_copy_glob(from, sp_str_lit("*"),
                    sp_os_join_path(to, sp_os_extract_file_name(from)));
...
sp_da(sp_fs_dir_ent_t) sp_fs_collect(sp_str_t path) {
  ...
  do {
    if (sp_cstr_equal(find_data.cFileName, "."))
      continue;
    if (sp_cstr_equal(find_data.cFileName, ".."))
      continue;
...
bool sp_fs_is_target_regular_file(sp_str_t path) {
  sp_stat_t st;
  if (sp_stat(sp_str_to_cstr(path), &st) != 0)
    return false;
  return sp_S_ISREG(st.st_mode);
}
...
sp_err_t sp_fs_copy(sp_str_t from, sp_str_t to) {
  if (sp_fs_is_glob(from)) {
    ...
    SP_ASSERT(sp_fs_is_target_dir(to));
    sp_fs_copy_glob(from, sp_str_lit("*"),
                    sp_fs_join_path(to, sp_fs_get_name(from)));
  } else if (sp_fs_is_target_regular_file(from)) {
    sp_fs_copy_file(from, to);
  }
...
void sp_fs_copy_file(sp_str_t from, sp_str_t to) {
  ...
  sp_stat_t stf;
  if (sp_stat(sp_str_to_cstr(from), &stf))
    return;
  ...
  sp_io_writer_t writer =
    sp_io_writer_from_file(to, SP_IO_WRITE_MODE_OVERWRITE);
  if (sp_err_get()) {
    return;
  }
...
sp_str_t sp_fs_get_storage_path() { return SP_ZERO_STRUCT(sp_str_t); }

sp_str_t sp_fs_get_config_path() { return SP_ZERO_STRUCT(sp_str_t); }
...
sp_str_t sp_os_try_xdg_or_home(sp_str_t xdg, sp_str_t home_suffix) {
  ...
  if (sp_str_valid(path))
    return sp_fs_join_path(path, home_suffix);
...
void sp_os_sleep_ms(f64 ms) { sp_os_sleep_ns((u64)(ms * 1000000.0)); }

c8* sp_os_wstr_to_cstr(c16* str16, u32 len) {
  s32 size_needed = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)str16, (s32)len,
                                        NULL, 0, NULL, NULL);
  c8* str8 = (c8*)sp_alloc((u32)(size_needed + 1));
  WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)str16, (s32)len, str8, size_needed,
                      NULL, NULL);
  str8[size_needed] = '\0';
  return str8;
}
...
sp_str_t sp_os_lib_to_file_name(sp_str_t lib_name, sp_os_lib_kind_t kind) {
  return sp_format("{}.{}", SP_FMT_STR(lib_name),
                   SP_FMT_STR(sp_os_lib_kind_to_extension(kind)));
}

sp_os_platform_kind_t sp_os_platform_kind() { return SP_OS_PLATFORM_WIN32; }

sp_str_t sp_os_platform_name() { return sp_str_lit("win32"); }

#elif defined(SP_FREESTANDING)
void sp_os_sleep_ns(u64 ns) {
  sp_sys_timespec_t remaining = {.tv_sec = (s64)(ns / 1000000000ULL),
...
void sp_os_sleep_ms(f64 ms) { sp_os_sleep_ns((u64)sp_tm_ms_to_ns_f(ms)); }
...
void sp_sleep_ns(u64 target) {
  ...
  // guard against elapsed > target (can happen with debugger pauses or OS
  // overshoots)
  if (elapsed < target) {
...
void sp_sleep_ms(f64 ms) { sp_sleep_ns((u64)sp_tm_ms_to_ns_f(ms)); }
...
void sp_os_print(sp_str_t message) { sp_write(1, message.data, message.len); }
...
#if defined(SP_MACOS)
sp_os_kind_t sp_os_get_kind() { return SP_OS_MACOS; }

sp_str_t sp_os_get_name() { return sp_str_lit("macos"); }
...
sp_str_t sp_os_lib_to_file_name(sp_str_t lib_name, sp_os_lib_kind_t kind) {
  return sp_format("lib{}.{}", SP_FMT_STR(lib_name),
                   SP_FMT_STR(sp_os_lib_kind_to_extension(kind)));
}
#endif
...
#if defined(SP_LINUX)
sp_os_kind_t sp_os_get_kind() { return SP_OS_LINUX; }

sp_str_t sp_os_get_name() { return sp_str_lit("linux"); }
...
sp_tm_epoch_t sp_tm_now_epoch() {
  ...
  return SP_RVAL(sp_tm_epoch_t){.seconds = unix_100ns / 10000000,
                                .nanoseconds =
                                  (u32)((unix_100ns % 10000000) * 100)};
}
...
sp_str_t sp_tm_epoch_to_iso8601(sp_tm_epoch_t time) {
  ...
  sp_str_builder_append_fmt(
    &builder, "{:04}-{:02}-{:02}T{:02}:{:02}:{:02}.{:03}Z",
    SP_FMT_S32(st.wYear), SP_FMT_S32(st.wMonth), SP_FMT_S32(st.wDay),
    SP_FMT_S32(st.wHour), SP_FMT_S32(st.wMinute), SP_FMT_S32(st.wSecond),
...
sp_tm_point_t sp_tm_now_point() {
  ...
  u64 ns =
    seconds * 1000000000ULL + (remainder * 1000000000ULL) / freq.QuadPart;
...
sp_tm_timer_t sp_tm_start_timer() {
  ...
  return SP_RVAL(sp_tm_timer_t){.start = now, .previous = now};
}
...
sp_tm_date_time_t sp_tm_get_date_time() {
  SYSTEMTIME st;
  GetLocalTime(&st);
  return SP_RVAL(sp_tm_date_time_t){.year = st.wYear,
                                    .month = st.wMonth,
                                    .day = st.wDay,
                                    .hour = st.wHour,
                                    .minute = st.wMinute,
...
sp_tm_epoch_t sp_fs_get_mod_time(sp_str_t file_path) {
  ...
  if (!GetFileAttributesEx(sp_str_to_cstr(file_path), GetFileExInfoStandard,
                           &fad)) {
...
sp_tm_date_time_t sp_tm_get_date_time() {
  ...
  return SP_RVAL(sp_tm_date_time_t){.year = time_info->tm_year + 1900,
...
sp_tm_epoch_t sp_tm_now_epoch() {
  ...
  return SP_RVAL(sp_tm_epoch_t){.s = (u64)ts.tv_sec, .ns = (u32)ts.tv_nsec};
}
...
u64 sp_tm_ms_to_s(u64 ms) { return ms / SP_TM_S_TO_MS; }
...
u64 sp_tm_us_to_s(u64 us) { return us / SP_TM_S_TO_US; }

u64 sp_tm_us_to_ms(u64 us) { return us / SP_TM_MS_TO_US; }
...
u64 sp_tm_ns_to_s(u64 ns) { return ns / SP_TM_S_TO_NS; }

u64 sp_tm_ns_to_ms(u64 ns) { return ns / SP_TM_MS_TO_NS; }

u64 sp_tm_ns_to_us(u64 ns) { return ns / SP_TM_US_TO_NS; }

f64 sp_tm_s_to_ms_f(f64 s) { return s * (f64)SP_TM_S_TO_MS; }

f64 sp_tm_s_to_us_f(f64 s) { return s * (f64)SP_TM_S_TO_US; }

f64 sp_tm_s_to_ns_f(f64 s) { return s * (f64)SP_TM_S_TO_NS; }

f64 sp_tm_ms_to_s_f(f64 ms) { return ms / (f64)SP_TM_S_TO_MS; }

f64 sp_tm_ms_to_us_f(f64 ms) { return ms * (f64)SP_TM_MS_TO_US; }

f64 sp_tm_ms_to_ns_f(f64 ms) { return ms * (f64)SP_TM_MS_TO_NS; }

f64 sp_tm_us_to_s_f(f64 us) { return us / (f64)SP_TM_S_TO_US; }

f64 sp_tm_us_to_ms_f(f64 us) { return us / (f64)SP_TM_MS_TO_US; }

f64 sp_tm_us_to_ns_f(f64 us) { return us * (f64)SP_TM_US_TO_NS; }

f64 sp_tm_ns_to_s_f(f64 ns) { return ns / (f64)SP_TM_S_TO_NS; }

f64 sp_tm_ns_to_ms_f(f64 ns) { return ns / (f64)SP_TM_MS_TO_NS; }

f64 sp_tm_ns_to_us_f(f64 ns) { return ns / (f64)SP_TM_US_TO_NS; }
...
sp_tm_epoch_t sp_fs_get_mod_time(sp_str_t file_path) {
  ...
  return SP_RVAL(sp_tm_epoch_t){.s = (u64)st.st_mtime,
...
sp_tm_epoch_t sp_fs_get_mod_time(sp_str_t file_path) {
  ...
  return SP_RVAL(sp_tm_epoch_t){.s = (u64)st.st_mtime_sec,
...
// ╚═╝  ╚═╝   ╚═╝    ╚═════╝ ╚═╝     ═╝╚═╝ ╚═════╝
// @atomic
bool sp_atomic_s32_cmp_and_swap(sp_atomic_s32* value, s32 current,
                                s32 desired) {
#if defined(SP_MSVC)
...
#elif defined(SP_POSIX)
void sp_semaphore_init(sp_semaphore_t* semaphore) { sem_init(semaphore, 0, 0); }

void sp_semaphore_destroy(sp_semaphore_t* semaphore) { sem_destroy(semaphore); }

void sp_semaphore_wait(sp_semaphore_t* semaphore) { sem_wait(semaphore); }
...
void sp_semaphore_signal(sp_semaphore_t* semaphore) { sem_post(semaphore); }
#endif
#endif
...
s32 sp_mutex_kind_to_c11(sp_mutex_kind_t kind) {
  s32 c11_kind;
  if (kind & SP_MUTEX_PLAIN)
    c11_kind = mtx_plain;
  if (kind & SP_MUTEX_TIMED)
    c11_kind = mtx_timed;
  if (kind & SP_MUTEX_RECURSIVE)
    c11_kind |= mtx_recursive;
...
void sp_mutex_lock(sp_mutex_t* mutex) { mtx_lock(mutex); }

void sp_mutex_unlock(sp_mutex_t* mutex) { mtx_unlock(mutex); }

void sp_mutex_destroy(sp_mutex_t* mutex) { mtx_destroy(mutex); }
#elif defined(SP_POSIX)
void sp_mutex_init(sp_mutex_t* mutex, sp_mutex_kind_t kind) {
...
void sp_mutex_lock(sp_mutex_t* mutex) { pthread_mutex_lock(mutex); }

void sp_mutex_unlock(sp_mutex_t* mutex) { pthread_mutex_unlock(mutex); }

void sp_mutex_destroy(sp_mutex_t* mutex) { pthread_mutex_destroy(mutex); }
#endif
#endif

#if defined(SP_CV)
#if defined(SP_POSIX)
void sp_cv_init(sp_cv_t* cond) { pthread_cond_init(cond, NULL); }

void sp_cv_destroy(sp_cv_t* cond) { pthread_cond_destroy(cond); }
...
void sp_cv_notify_one(sp_cv_t* cond) { pthread_cond_signal(cond); }

void sp_cv_notify_all(sp_cv_t* cond) { pthread_cond_broadcast(cond); }

#elif defined(SP_WIN32)
void sp_cv_init(sp_cv_t* cond) { InitializeConditionVariable(cond); }

void sp_cv_destroy(sp_cv_t* cond) { SP_UNUSED(cond); }
...
void sp_cv_notify_one(sp_cv_t* cond) { WakeConditionVariable(cond); }

void sp_cv_notify_all(sp_cv_t* cond) { WakeAllConditionVariable(cond); }
#endif
#endif
...
void sp_thread_init(sp_thread_t* thread, sp_thread_fn_t fn, void* userdata) {
  sp_thread_launch_t launch =
    SP_RVAL(sp_thread_launch_t){.fn = fn,
                                .userdata = userdata,
...
void sp_thread_join(sp_thread_t* thread) { pthread_join(*thread, NULL); }
...
void sp_os_clear_env_var(sp_str_t key) { unsetenv(sp_str_to_cstr(key)); }

void sp_os_export_env(sp_env_t* env, sp_env_export_t overwrite) {
  sp_ht_for(env->vars, it) {
    sp_os_export_env_var(*sp_ht_it_getkp(env->vars, it),
                         *sp_ht_it_getp(env->vars, it), overwrite);
  }
}

void sp_os_export_env_var(sp_str_t key, sp_str_t value,
                          sp_env_export_t overwrite) {
...
bool sp_ps_is_fd_valid(sp_os_file_handle_t fd) { return fd > 0; }
...
void sp_ps_free_posix_args(c8** args) { sp_dyn_array_free(args); }
...
c8** sp_ps_build_posix_env(sp_ps_env_config_t* config) {
  ...
  for (u32 i = 0; i < SP_PS_MAX_ENV; i++) {
    if (sp_str_empty(config->extra[i].key))
      break;
  ...
  sp_ht_for(env.vars, it) {
    ...
    sp_str_builder_t builder = SP_ZERO_INITIALIZE();
    sp_str_builder_append_fmt(&builder, "{}={}", SP_FMT_STR(key),
                              SP_FMT_STR(val));
...
void sp_ps_configure_io_in(sp_ps_io_in_config_t* io,
                           sp_ps_stdio_config_entry_t* p) {
  switch (io->mode) {
  case SP_PS_IO_MODE_NULL: {
    SP_ASSERT(posix_spawn_file_actions_addopen(
                p->fa, p->file_number, "/dev/null", p->flag, p->mode) == 0);
    break;
  }
  case SP_PS_IO_MODE_CREATE: {
    ...
    SP_ASSERT(posix_spawn_file_actions_adddup2(p->fa, p->pipes.read,
                                               p->file_number) == 0);
    break;
  }
  case SP_PS_IO_MODE_EXISTING: {
    SP_ASSERT(io->fd);
    SP_ASSERT(posix_spawn_file_actions_adddup2(p->fa, io->fd, p->file_number) ==
              0);
    break;
  }
...
void sp_ps_configure_io_out(sp_ps_io_out_config_t* io,
                            sp_ps_stdio_config_entry_t* p) {
  switch (io->mode) {
  case SP_PS_IO_MODE_NULL: {
    SP_ASSERT(posix_spawn_file_actions_addopen(
  ...
  case SP_PS_IO_MODE_CREATE: {
    ...
    p->pipes.read = pipes[0];
    p->pipes.write = pipes[1];
    SP_ASSERT(posix_spawn_file_actions_adddup2(p->fa, p->pipes.write,
                                               p->file_number) == 0);
    break;
  }
  case SP_PS_IO_MODE_EXISTING: {
  ...
  case SP_PS_IO_MODE_REDIRECT: {
    s32 redirect = p->file_number == SP_PS_IO_FILENO_STDOUT
                     ? SP_PS_IO_FILENO_STDERR
                     : SP_PS_IO_FILENO_STDOUT;
    SP_ASSERT(
      posix_spawn_file_actions_adddup2(p->fa, redirect, p->file_number) == 0);
    break;
  }
  case SP_PS_IO_MODE_INHERIT: {
    break;
...
sp_ps_t sp_ps_create(sp_ps_config_t config) {
  ...
  sp_ps_stdio_config_t io = {
    .in = (sp_ps_stdio_config_entry_t){.fa = &fa,
  ...
  if (posix_spawnp(&pid, argv[0], &fa, &attr, argv, envp) != 0) {
    ...
    if (io.in.pipes.read >= 0) {
      ...
      close(io.in.pipes.write);
    }
    if (io.out.pipes.read >= 0) {
      close(io.out.pipes.read);
      close(io.out.pipes.write);
    }
    if (io.err.pipes.read >= 0) {
      close(io.err.pipes.read);
  ...
  if (io.in.pipes.read >= 0) {
    ...
    case SP_PS_IO_NONBLOCKING: {
      sp_ps_set_nonblocking(io.in.pipes.write);
      break;
    }
    case SP_PS_IO_BLOCKING: {
      sp_ps_set_blocking(io.in.pipes.write);
      break;
    }
    }
  ...
  if (io.out.pipes.read >= 0) {
    ...
    case SP_PS_IO_NONBLOCKING: {
      sp_ps_set_nonblocking(io.out.pipes.read);
      break;
    }
    case SP_PS_IO_BLOCKING: {
      sp_ps_set_blocking(io.out.pipes.read);
      break;
    }
    }
  ...
  if (io.err.pipes.read >= 0) {
    ...
    case SP_PS_IO_NONBLOCKING: {
      sp_ps_set_nonblocking(io.err.pipes.read);
      break;
    }
    case SP_PS_IO_BLOCKING: {
      sp_ps_set_blocking(io.err.pipes.read);
      break;
    }
    }
...
sp_ps_output_t sp_ps_run(sp_ps_config_t config) {
  if (config.io.out.mode == SP_PS_IO_MODE_EXISTING ||
      config.io.out.mode == SP_PS_IO_MODE_REDIRECT) {
    SP_FATAL("You called sp_ps_run() but your config redirected stdout. "
             "sp_ps_run() always creates a new "
             "file descriptor for stdout, since its purpose is to run a "
             "command and capture output without "
             "the command polluting the parent command's output. If you really "
             "wanted this, use sp_ps_create() "
...
sp_io_writer_t* sp_ps_io_in(sp_ps_t* ps) {
  if (!ps)
    return SP_NULLPTR;
  if (!sp_ps_is_fd_valid(ps->io.in.fd))
    return SP_NULLPTR;

  sp_io_writer_t* writer = sp_alloc_type(sp_io_writer_t);
  *writer =
    sp_io_writer_from_fd(ps->io.in.fd, sp_ps_io_close_mode(ps->io.in.mode));
  return writer;
}
...
sp_io_reader_t* sp_ps_io_out(sp_ps_t* ps) {
  if (!ps)
    return SP_NULLPTR;
  if (!sp_ps_is_fd_valid(ps->io.out.fd))
    return SP_NULLPTR;

  sp_io_reader_t* reader = sp_alloc_type(sp_io_reader_t);
  *reader =
    sp_io_reader_from_fd(ps->io.out.fd, sp_ps_io_close_mode(ps->io.out.mode));
  return reader;
}
...
sp_io_reader_t* sp_ps_io_err(sp_ps_t* ps) {
  if (!ps)
    return SP_NULLPTR;
  if (!sp_ps_is_fd_valid(ps->io.err.fd))
    return SP_NULLPTR;

  sp_io_reader_t* reader = sp_alloc_type(sp_io_reader_t);
  *reader =
    sp_io_reader_from_fd(ps->io.err.fd, sp_ps_io_close_mode(ps->io.err.mode));
  return reader;
}
...
sp_ps_status_t sp_ps_poll(sp_ps_t* ps, u32 timeout_ms) {
  ...
  do {
    wait_result =
      sp_wait4(ps->pid, &wait_status, SP_POSIX_WAITPID_NO_BLOCK, SP_NULLPTR);
    if (wait_result == 0) {
...
sp_ps_status_t sp_ps_wait(sp_ps_t* ps) {
  ...
  do {
    wait_result =
      sp_wait4(ps->pid, &wait_status, SP_POSIX_WAITPID_BLOCK, SP_NULLPTR);
...
void sp_fmon_init(sp_fmon_t* monitor, sp_fmon_fn_t callback,
                  sp_fmon_event_kind_t events, void* userdata) {
  sp_fmon_init_ex(monitor, callback, events, userdata, 0,
                  sp_context_get()->allocator);
}

void sp_fmon_init_ex(sp_fmon_t* monitor, sp_fmon_fn_t callback,
                     sp_fmon_event_kind_t events, void* userdata, u32 debounce,
                     sp_allocator_t alloc) {
...
void sp_fmon_deinit(sp_fmon_t* monitor) { sp_fmon_os_deinit(monitor); }
...
sp_fmon_cache_t* sp_fmon_get_or_insert_cache(sp_fmon_t* monitor,
                                             sp_str_t file_path) {
...
#ifdef SP_WIN32
SP_PRIVATE void sp_os_win32_file_monitor_add_change(sp_fmon_t* monitor,
                                                    sp_str_t file_path,
                                                    sp_str_t file_name,
                                                    sp_fmon_event_t events);
SP_PRIVATE void sp_os_win32_file_monitor_issue_one_read(sp_fmon_t* monitor,
                                                        sp_fmon_dir_t* info);

void sp_fmon_os_init(sp_file_monitor_t* monitor) {
  sp_os_win32_file_monitor_t* os =
    (sp_os_win32_file_monitor_t*)sp_alloc(sizeof(sp_os_win32_file_monitor_t));
  sp_dynamic_array_init(&os->directory_infos, sizeof(sp_monitored_dir_t));
  monitor->os = os;
}

void sp_fmon_os_deinit(sp_file_monitor_t* monitor) { (void)monitor; }
...
void sp_fmon_os_add_dir(sp_file_monitor_t* monitor, sp_str_t directory_path) {
  ...
  sp_win32_handle_t handle = CreateFileA(
    directory_cstr, FILE_LIST_DIRECTORY,
  ...
  sp_monitored_dir_t* info =
    (sp_monitored_dir_t*)sp_dynamic_array_push(&os->directory_infos, NULL);
...
void sp_fmon_os_process_changes(sp_file_monitor_t* monitor) {
  ...
  for (u32 i = 0; i < os->directory_infos.size; i++) {
    sp_monitored_dir_t* info =
      (sp_monitored_dir_t*)sp_dynamic_array_at(&os->directory_infos, i);
    ...
    s32 bytes_written = 0;
    bool success = GetOverlappedResult(info->handle, &info->overlapped,
                                       (LPDWORD)&bytes_written, false);
    if (!success || bytes_written == 0)
      break;

    FILE_NOTIFY_INFORMATION* notify =
      (FILE_NOTIFY_INFORMATION*)info->notify_information;
    while (true) {
      ...
      c8* partial_path_cstr = sp_wstr_to_cstr(
        &notify->FileName[0], (u32)(notify->FileNameLength / 2));
      ...
      sp_os_win32_file_monitor_add_change(monitor, full_path, file_name,
                                          events);

      if (notify->NextEntryOffset == 0)
        break;
      notify =
        (FILE_NOTIFY_INFORMATION*)((char*)notify + notify->NextEntryOffset);
    }
...
void sp_os_win32_file_monitor_add_change(sp_file_monitor_t* monitor,
                                         sp_str_t file_path, sp_str_t file_name,
                                         sp_file_change_event_t events) {
  f32 time = (f32)(GetTickCount64() / 1000.0);

  if (sp_os_is_directory(file_path))
    return;

  if (file_name.data && file_name.len > 0) {
    if (file_name.data[0] == '.' && file_name.len > 1 &&
        file_name.data[1] == '#')
      return;
    if (file_name.data[0] == '#')
      return;
  ...
  if (!sp_file_monitor_check_cache(monitor, file_path, time))
    return;

  for (u32 i = 0; i < monitor->changes.size; i++) {
    sp_file_change_t* change =
      (sp_file_change_t*)sp_dynamic_array_at(&monitor->changes, i);
  ...
  sp_file_change_t* change =
    (sp_file_change_t*)sp_dynamic_array_push(&monitor->changes, NULL);
...
void sp_os_win32_file_monitor_issue_one_read(sp_file_monitor_t* monitor,
                                             sp_monitored_dir_t* info) {
  SP_ASSERT(info->handle != INVALID_HANDLE_VALUE);

  s32 notify_filter = 0;
  if (monitor->events_to_watch &
      (SP_FILE_CHANGE_EVENT_ADDED | SP_FILE_CHANGE_EVENT_REMOVED)) {
  ...
  ReadDirectoryChangesW(info->handle, info->notify_information,
                        SP_FILE_MONITOR_BUFFER_SIZE, true, notify_filter, NULL,
    ...
    sp_dyn_array_for(os->watch_descs, it) {
      ...
      if (wd == event->wd) {
        ...
        if (event->mask & (SP_IN_MODIFY | SP_IN_ATTRIB | SP_IN_CLOSE_WRITE)) {
          events =
            (sp_fmon_event_kind_t)(events | SP_FILE_CHANGE_EVENT_MODIFIED);
        }
        ...
        if (event->mask & (SP_IN_DELETE | SP_IN_MOVED_FROM)) {
          events =
            (sp_fmon_event_kind_t)(events | SP_FILE_CHANGE_EVENT_REMOVED);
        }
...
#if defined(SP_FMON_MACOS_USE_FSEVENTS)
SP_PRIVATE void
sp_fmon_fsevents_callback(ConstFSEventStreamRef stream, void* user_data,
...
SP_PRIVATE void sp_fmon_fsevents_recreate_stream(sp_fmon_t* monitor) {
...

  {
    ...
    sp_da_for(os->watch_paths, it) {
      cf_paths[it] = CFStringCreateWithCString(
        kCFAllocatorDefault, sp_str_to_cstr(os->watch_paths[it]),
        kCFStringEncodingUTF8);
    }

    paths = CFArrayCreate(NULL, (const void**)cf_paths, num_paths,
                          &kCFTypeArrayCallBacks);

    sp_for(it, num_paths) { CFRelease(cf_paths[it]); }
  ...
  os->stream = FSEventStreamCreate(
    kCFAllocatorDefault, sp_fmon_fsevents_callback, &context, paths,
...
void sp_fmon_os_deinit(sp_fmon_t* monitor) {
  ...
  sp_da_for(os->fds, it) { close(os->fds[it]); }
  if (os->kq > 0) {
    close(os->kq);
  }
}
    ...
    sp_da_for(os->fds, it) {
      if (os->fds[it] == fd) {
        ...
        if ((monitor->events_to_watch & SP_FILE_CHANGE_EVENT_MODIFIED) &&
          ...
          event_kind =
            (sp_fmon_event_kind_t)(event_kind | SP_FILE_CHANGE_EVENT_MODIFIED);
        }
        if ((monitor->events_to_watch & SP_FILE_CHANGE_EVENT_ADDED) &&
            (ev->fflags & NOTE_LINK)) {
          event_kind =
            (sp_fmon_event_kind_t)(event_kind | SP_FILE_CHANGE_EVENT_ADDED);
        }
        if ((monitor->events_to_watch & SP_FILE_CHANGE_EVENT_REMOVED) &&
            (ev->fflags & (NOTE_DELETE | NOTE_RENAME | NOTE_REVOKE))) {
          event_kind =
            (sp_fmon_event_kind_t)(event_kind | SP_FILE_CHANGE_EVENT_REMOVED);
        }

        if (event_kind != SP_FILE_CHANGE_EVENT_NONE) {
          sp_fmon_event_t change = {.file_path = sp_str_copy(path),
...
#elif defined(SP_COSMO)
void sp_fmon_os_init(sp_fmon_t* monitor) { (void)monitor; }

void sp_fmon_os_deinit(sp_fmon_t* monitor) { (void)monitor; }
...
void sp_fmon_os_process_changes(sp_fmon_t* monitor) { (void)monitor; }
#endif
#endif
...
sp_err_t sp_err_get() { return sp_context_get()->err.sp; }

sp_os_err_t sp_err_get_os() { return sp_context_get()->err.os; }

sp_err_ext_t sp_err_get_ext() { return sp_context_get()->err; }
...
sp_str_t sp_io_read_file(sp_str_t path) {
  ...
  return (sp_str_t){.data = data, .len = (u32)bytes_read};
}
...
s64 sp_io_reader_file_seek(sp_io_reader_t* r, s64 offset,
                           sp_io_whence_t whence) {
...
s64 sp_io_reader_mem_seek(sp_io_reader_t* r, s64 offset,
                          sp_io_whence_t whence) {
  s64 pos = 0;
...
u64 sp_io_reader_mem_size(sp_io_reader_t* r) { return r->mem.len; }

void sp_io_reader_mem_close(sp_io_reader_t* r) { (void)r; }
...
sp_io_reader_t sp_io_reader_from_fd(sp_os_file_handle_t fd,
                                    sp_io_close_mode_t mode) {
...
u64 sp_io_read(sp_io_reader_t* reader, void* ptr, u64 size) {
  ...
  reader->seek = 0;
  reader->buffer.len =
    reader->vtable.read(reader, reader->buffer.data, reader->buffer.capacity);
...
s64 sp_io_reader_seek(sp_io_reader_t* reader, s64 offset,
                      sp_io_whence_t whence) {
  if (!reader)
    return 0;
  if (!reader->vtable.seek)
    return 0;
...
s64 sp_io_writer_file_seek(sp_io_writer_t* writer, s64 offset,
                           sp_io_whence_t whence) {
...
void sp_io_writer_file_close(sp_io_writer_t* writer) {
  if (writer->file.close_mode != SP_IO_CLOSE_MODE_AUTO)
    return;
...
s64 sp_io_writer_mem_seek(sp_io_writer_t* writer, s64 offset,
                          sp_io_whence_t whence) {
  s64 pos = 0;
...
u64 sp_io_writer_mem_size(sp_io_writer_t* writer) { return writer->mem.len; }

void sp_io_writer_mem_close(sp_io_writer_t* writer) { (void)writer; }
...
u64 sp_io_writer_dyn_write(sp_io_writer_t* writer, const void* ptr, u64 size) {
  ...
  if (required > io->buffer.capacity) {
    ...
    io->buffer.data = (u8*)sp_mem_allocator_realloc(
      io->allocator, io->buffer.data, (u32)new_capacity);
...
s64 sp_io_writer_dyn_seek(sp_io_writer_t* writer, s64 offset,
                          sp_io_whence_t whence) {
  sp_io_writer_dyn_mem_t* io = &writer->dyn_mem;
  s64 pos = 0;
...
void sp_io_writer_dyn_close(sp_io_writer_t* writer) {
  if (writer->dyn_mem.buffer.data) {
    sp_mem_allocator_free(writer->dyn_mem.allocator,
                          writer->dyn_mem.buffer.data);
...
sp_io_writer_t sp_io_writer_from_dyn_mem() {
  return sp_io_writer_from_dyn_mem_ex(SP_NULLPTR, 0,
                                      sp_context_get()->allocator);
}

sp_io_writer_t sp_io_writer_from_dyn_mem_ex(u8* buffer, u64 size,
                                            sp_allocator_t allocator) {
  return (sp_io_writer_t){
    ...
    .dyn_mem = {.allocator = allocator,
                .buffer = {.data = buffer, .len = 0, .capacity = size}}};
}
...
void sp_io_writer_set_buffer(sp_io_writer_t* writer, u8* ptr, u64 size) {
  ...
  writer->buffer = (sp_mem_buffer_t){.data = ptr, .len = 0, .capacity = size};
}

sp_err_t sp_io_flush(sp_io_writer_t* writer) {
  if (!writer)
    return SP_ERR_OK;
  if (!writer->vtable.write)
    return SP_ERR_OK;
  ...
  while (written < writer->buffer.len) {
    ...
    u64 result =
      writer->vtable.write(writer, writer->buffer.data + written, remaining);
    if (result == 0) {
      if (written > 0) {
        sp_mem_move(writer->buffer.data + written, writer->buffer.data,
...
s64 sp_io_writer_seek(sp_io_writer_t* writer, s64 offset,
                      sp_io_whence_t whence) {
  if (!writer)
    return 0;
  if (!writer->vtable.seek)
    return 0;
  sp_io_flush(writer);
  sp_err_clear();
...
// @asset
#if defined(SP_ASSET)
void sp_asset_registry_init(sp_asset_registry_t* registry,
                            sp_asset_registry_config_t config) {
  ...
  for (u32 index = 0; index < SP_ASSET_REGISTRY_CONFIG_MAX_IMPORTERS; index++) {
    ...
    if (cfg->kind == SP_ASSET_KIND_NONE)
      break;
...
sp_asset_t* sp_asset_registry_add(sp_asset_registry_t* registry,
                                  sp_asset_kind_t kind, sp_str_t name,
                                  void* user_data) {
  sp_mutex_lock(&registry->mutex);
...
sp_future_t* sp_asset_registry_import(sp_asset_registry_t* registry,
                                      sp_asset_kind_t kind, sp_str_t name,
                                      void* user_data) {
  sp_asset_importer_t* importer =
    sp_asset_registry_find_importer(registry, kind);
  SP_ASSERT(importer);
  ...
  u32 asset_index = sp_dyn_array_size(registry->assets) -
                    1; // Get index of the asset we just added
  sp_mutex_unlock(&registry->mutex);
...
sp_asset_t* sp_asset_registry_find(sp_asset_registry_t* registry,
                                   sp_asset_kind_t kind, sp_str_t name) {
  sp_mutex_lock(&registry->mutex);
...
sp_asset_importer_t*
sp_asset_registry_find_importer(sp_asset_registry_t* registry,
                                sp_asset_kind_t kind) {
  sp_mutex_lock(&registry->mutex);
...
sp_app_t* sp_app_new(sp_app_config_t config) {
  ...
  *app = (sp_app_t){.user_data = config.user_data,
...
#if defined(SP_ELF)
bool sp_elf_is_align_valid(u64 v) { return v == 0 || (v & (v - 1)) == 0; }

sp_elf_t* sp_elf_new() { return sp_elf_new_alloc(sp_context_get()->allocator); }
...
sp_elf_t* sp_elf_new_alloc(sp_allocator_t allocator) {
  ...
  sp_ht_set_fns(elf->section_map, sp_ht_on_hash_str_key,
...
sp_elf_section_t* sp_elf_add_section(sp_elf_t* elf, sp_str_t name, u32 kind,
                                     u64 alignment) {
...
sp_elf_section_t* sp_elf_symtab_new(sp_elf_t* elf) {
  ...
  u32 strtab_idx =
    sp_elf_add_section(elf, sp_str_lit(".strtab"), SHT_STRTAB, 1)->index;
  u32 symtab_idx =
    sp_elf_add_section(elf, sp_str_lit(".symtab"), SHT_SYMTAB, 8)->index;
...
sp_elf_section_t* sp_elf_rela_new(sp_elf_t* elf, sp_elf_section_t* target) {
  ...
  sp_elf_section_t* symtab =
    sp_elf_find_section_by_name(elf, sp_str_lit(".symtab"));
  if (!symtab)
    return SP_NULLPTR;
...
void sp_elf_add_relocation(sp_elf_section_t* rela, u64 offset, u32 sym_idx,
                           u32 type, s64 addend) {
  if (!rela)
    return;
...
sp_elf_section_t* sp_elf_find_or_create_shstrtab(sp_elf_t* elf) {
  sp_elf_section_t* table =
    sp_elf_find_section_by_name(elf, sp_str_lit(".shstrtab"));
  if (table) {
    return table;
  }
...
sp_elf_t* sp_elf_read(sp_io_reader_t* in) {
  ...
  bytes_read =
    sp_io_read(in, section_headers, num_sections * sizeof(Elf64_Shdr));
  ...
  sp_da_push(elf->sections, ((sp_elf_section_t){.type = SHT_NULL}));
  ...
  for (u32 it = 1; it < num_sections; it++) {
    ...
    sp_elf_section_t* section =
      sp_elf_add_section(elf, name, header->sh_type, header->sh_addralign);
...
sp_macho_section_t* sp_macho_get_section(sp_macho_t* m, u32 index) {
  if (!m)
    return SP_NULLPTR;
  if (index == 0 || index > sp_da_size(m->sections))
    return SP_NULLPTR;
...
sp_err_t sp_macho_write(sp_macho_t* m, sp_io_writer_t* out) {
  ...
  u32 ncmds = 1;
  u32 sizeofcmds =
    sizeof(struct segment_command_64) + nsects * sizeof(struct section_64);
  ...
  u64 strtab_size = 2;
  sp_da_for(m->symbols, i) { strtab_size += m->symbols[i].name.len + 2; }
  ...
  struct segment_command_64 seg = {
    .cmd = LC_SEGMENT_64,
    .cmdsize = (u32)(sizeof(struct segment_command_64) +
  ...
  sp_da_for(m->symbols, i) {
    ...
    u32 strx = 2;
    sp_for(j, i) { strx += (u32)m->symbols[j].name.len + 2; }
  ...
  sp_da_for(m->symbols, i) { strtab_written += m->symbols[i].name.len + 2; }
