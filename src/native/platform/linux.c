...
static int add_path_rule(int ruleset_fd, const char* path, __u64 access,
  ...
  if (ret < 0) {
    snprintf(errbuf, errbuf_sz, "landlock_add_rule(%s): %s", path,
             strerror(saved));
    return -1;
  }
  return 0;
...
static int build_ruleset(const struct sb* sb) {
  ...
  if (add_path_rule(ruleset_fd, "/", read_access, (char*)sb->errbuf,
                    sizeof(sb->errbuf)) < 0) {
    close(ruleset_fd);
    return -1;
  }
  ...
  if (add_path_rule(ruleset_fd, "/dev", full_access, (char*)sb->errbuf,
                    sizeof(sb->errbuf)) < 0) {
    close(ruleset_fd);
    return -1;
  }
