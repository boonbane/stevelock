...
static napi_value n_create(napi_env env, napi_callback_info info) {
  ...
  if (napi_get_named_property(env, opts_obj, "readable", &v_readable) ==
      napi_ok) {
    bool is_arr = false;
  ...
  n_sb_handle_t* h = calloc(1, sizeof(*h));
  if (!h) {
    sb_destroy(sb);
    napi_throw_error(env, NULL, "failed to allocate sandbox handle");
    return NULL;
  }
  h->sb = sb;

  napi_value external;
  NAPI_CALL(napi_create_external(env, h, n_finalize, NULL, &external));
  return external;
}
...
static napi_value n_spawn(napi_env env, napi_callback_info info) {
  ...
  size_t cmd_len;
  NAPI_CALL(
    napi_get_value_string_utf8(env, argv[1], cmd, sizeof(cmd), &cmd_len));
  ...
  if (is_arr)
    napi_get_array_length(env, argv[2], &args_len);
